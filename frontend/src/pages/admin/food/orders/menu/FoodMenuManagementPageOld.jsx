...
const FoodMenuManagementPage = () => {
  ...
  const handleAIMenuExtract = async () => {
    ...
    try {
      let result;
      if (aiMenuImageFile) {
        result = await foodService.extractMenuFromImage(aiMenuImageFile);
      } else if (aiMenuImageUrl) {
        setAIExtractError('Image URL extraction not currently supported. Please upload an image file.');
        setAIExtracting(false);
        return;
      } else {
        setAIExtractError('Please upload an image file.');
        setAIExtracting(false);
        return;
      }
      if (result && result.data && Array.isArray(result.data.menuItems)) {
        setAIExtractedItems(result.data.menuItems);
      } else {
        setAIExtractError('No menu items extracted. Please try another image.');
      }
    } catch (err) {
