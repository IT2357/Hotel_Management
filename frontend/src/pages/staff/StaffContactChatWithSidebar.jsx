...
const StaffContactChat = () => {
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      // Determine department from user profile
      ...
      console.log('📤 Sending message to:', `${import.meta.env.VITE_API_URL || 'http://localhost:5000'}/api/staff/messaging/send-to-manager`);
      ...
      if (response.ok) {
        ...
        const errorMessage = data.message || data.error || `Failed to send message (${response.status})`;
            ...
            <div className="bg-gray-50 px-4 py-3 border-b border-gray-200">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2 bg-white border border-gray-300 rounded-lg focus:outline-none focus:border-green-500 text-sm"
                />
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto">
              {filteredManagers.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-gray-500">
                  <Users className="w-16 h-16 mb-2 text-gray-300" />
                ...
                filteredManagers.map((manager) => (
                  <div
                    ...
                    className={`flex items-center p-4 hover:bg-gray-50 cursor-pointer transition border-b border-gray-100 ${
                      selectedManager?._id === manager._id ? 'bg-green-50' : ''
                    }`}
                  >
                    <div className="relative">
                    ...
                    <div className="ml-3 flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        ...
                        {unreadCount > 0 && selectedManager?._id === manager._id && (
                      ...
                      <p className="text-xs text-gray-400 mt-1">{manager.role}</p>
                    </div>
                  </div>
                ))
              )}
      ...
      <div className="flex-1 flex flex-col">
        {!selectedManager ? (
          <div className="flex-1 flex flex-col items-center justify-center bg-gray-50">
            ...
            <h2 className="text-2xl font-semibold text-gray-600 mb-2">Welcome to Staff Chat</h2>
            <p className="text-gray-500">Select a manager to start messaging</p>
          </div>
        ) : (
          <>
            {/* Chat Header */}
            <div className="bg-green-600 text-white shadow-md">
              <div className="flex items-center justify-between px-4 py-3">
                <div className="flex items-center space-x-3">
                  ...
                  <button 
                    onClick={() => setShowSidebar(!showSidebar)}
                    className="hidden md:block hover:bg-green-700 p-2 rounded-full transition"
                  >
                  ...
                  <div className="relative">
                    <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center text-green-600 font-semibold">
                      {selectedManager?.name?.[0]?.toUpperCase() || 'M'}
                    </div>
                    {isOnline && (
                  ...
                  <div>
                    <h2 className="font-semibold text-base">{selectedManager?.name || 'Manager'}</h2>
