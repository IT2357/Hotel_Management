/**
 * 🤖 AIMenuUploader 2025 - OCR Menu Extraction Workflow
 * Features: Drag-drop upload, progress tracking, AI extraction with confidence scores
 * Real-world: Editable results, category correction, bulk save, #FF9933 theme
 */

import React, { useState } from 'react';
import { Upload, FileImage, X, Check, Edit2, Save, AlertCircle, Sparkles, CheckCircle } from 'lucide-react';
import { aiAPI, menuAPI, categoryAPI } from '../services/apiService';

const AIMenuUploader = () => {
  ...
  const handleExtract = async () => {
    ...
    try {
      ...
      const response = await aiAPI.extractMenu(selectedFile, (progress) => {
  ...
  const handleSaveAll = async () => {
    try {
      ...
      const promises = extractedItems.map(async (item) => {
        ...
        if (item.description_tamil) formData.append('description_tamil', item.description_tamil);
        if (item.description_english) formData.append('description_english', item.description_english);
        if (item.ingredients) {
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Upload a Jaffna menu image and let AI extract the items automatically
          </p>
        </div>
          ...
          {selectedFile && !uploading && extractedItems.length === 0 && (
            <div className="mt-6 text-center">
              <button
                ...
                className="px-8 py-4 bg-gradient-to-r from-[#FF9933] to-[#FF7700] text-white rounded-lg hover:shadow-xl transition-all font-bold text-lg flex items-center gap-2 mx-auto"
              >
          ...
          {uploading && (
            <div className="mt-6">
              ...
              <p className="text-xs text-gray-500 mt-2 text-center">
                Processing with Tamil + English OCR...
              </p>
            </div>
          )}
