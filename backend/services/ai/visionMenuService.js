...
async function geminiProvider({ imageBuffer, mimeType, ocrText }) {
  ...
  try {
    ...
    // Use a stable, supported model alias. 'gemini-1.5-flash-latest' is generally available.
...
`;
    ...
    try {
      ...
      // If the chosen model is not available, fallback to a pro model alias
      console.warn('Gemini primary model failed, falling back to pro-latest:', err?.message || err);
